...
element_t * Ajout_tete_ensemble(element_t *ensemble, int val, int freq) {
    element_t* tmp;
    element_t* cursor = Recherche_val(ensemble,val);
    if(cursor==NULL) {
        ...
        cursor->frequence = cursor->frequence + freq;
        return ensemble;
    }
}

element_t *Supprime_total_element(element_t * ensemble, int val) {
    element_t *p = ensemble;
    element_t *tmp;
    while(p->valeur != val) {
        if(p == NULL) return ensemble;
        if(p->suivant->valeur == val) tmp = p;
        p = p->suivant;
    }
...
element_t* Supprime_total_element1(element_t* ensemble, int val) {
  if(Recherche_val(ensemble,val)==NULL) return ensemble;
  element_t* p = Recherche_val(ensemble,val);
